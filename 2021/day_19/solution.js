export default function learn(time, courses) {
  ...
  const possibleCombinations = combinations.filter(
    ([a, b]) => courses[a] + courses[b] <= time
  );

  if (!possibleCombinations.length) return null;

  return possibleCombinations.reduce((best, current) =>
    courses[current[0]] + courses[current[1]] >
    courses[best[0]] + courses[best[1]]
      ? current
      : best
  );
}
